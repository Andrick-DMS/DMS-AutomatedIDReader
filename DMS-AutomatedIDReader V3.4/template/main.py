...
def esta_vetado(cedula, archivo="vetados.xlsx"):
    try:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            if row and str(row[0]).strip() == cedula.strip():
                return True
        return False
    except Exception as e:
        print(f"⚠️ Error revisando vetados: {e}")
        return False

def agregar_a_vetados(cedula, archivo="vetados.xlsx"):
    try:
        if not os.path.exists(archivo):
        ...
        ws.append([cedula])
        wb.save(archivo)
        guardar_log(f"🟡 Cedula agregada manualmente a vetados: {cedula}")
    except Exception as e:
        print(f"⚠️ Error al agregar a vetados: {e}")
        ...
        with serial.Serial(puerto, baudrate=9600, timeout=1) as ser:
            while True:
                ...
                if len(buffer) >= 600:
                    ...
                    nombre_completo = f"{datos.get('Nombre', '')} {datos.get('Primer Apellido', '')} {datos.get('Segundo Apellido', '')}"

                    if esta_vetado(cedula):
                        msg = f"🚨 INVITADO VETADO - {nombre_completo} - Cédula: {cedula}"
                        guardar_log(msg)
                        top = Tk(); top.withdraw()
                        messagebox.showwarning("INVITADO VETADO", "Invitado vetado. Llamar a seguridad.", parent=top)
                        top.destroy()
                        continue

                    estado_actual = estado_visitas.get(cedula)

                    if estado_actual is None or estado_actual == "salida":
                        # Permitir ingreso
                        guardar_log(f"🟢 Entrada de visitante - {nombre_completo} - Cédula: {cedula}")
                        config = cargar_configuracion_activa()
                        if config:
                            escribir_con_configuracion(datos, config)
                        estado_visitas[cedula] = "entrada"

                    elif estado_actual == "entrada":
                        top = Tk(); top.withdraw()
                        respuesta = messagebox.askyesno("Confirmar salida", f"{nombre_completo}\n\n¿Deseas agregar a este visitante a la lista de vetados?", parent=top)
                        top.destroy()
                        if respuesta:
                            agregar_a_vetados(cedula)
                            guardar_log(f"🔴 Salida de invitado vetado - {nombre_completo} - Cédula: {cedula}")
                        else:
                            guardar_log(f"🟡 Salida de invitado - {nombre_completo} - Cédula: {cedula}")
                            estado_visitas[cedula] = "salida"
    except Exception as e:
        print(f"Error escuchando lector QR: {e}")
